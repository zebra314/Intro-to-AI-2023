...
def ucs(start, end):
    ...
    """
    Init the containers.
    :que: a list of string, storing the nodes while implementing UCS.
    ...
    """
    Implement UCS.
    - End the loop if the queue is empty or the end node is found.
    - In the loop:
        - Take out the node with the smallest distance from the queue.
        ...
        - For each node, calculate the distance from the start node to the node.
        - If the node is not in the visited set or the distance is smaller than the distance
            recorded in the parent dictionary, update the parent dictionary and put the node
            into the queue.
    """
    ...
    - In the loop:
        ...
        - If the current node is the start node, end the loop.
        - Add the distance from the current node to its parent node to the total distance.
